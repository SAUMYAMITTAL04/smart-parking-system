...
app_mode = st.sidebar.radio("Select Interface View:", [
    "📊 Manager Operations Center", 
    "📈 Executive BI Analytics", 
    "📱 Driver Kiosk Mobile View"
])
...
# ==========================================
# 📈 VIEW 2: EXECUTIVE BI ANALYTICS
# ==========================================
elif app_mode == "📈 Executive BI Analytics":
    ...
    if not df_logs.empty:
        ...
        with col_bi2:
            ...
            st.dataframe(rev_summary, width="stretch")
...
# ==========================================
    ...
    # 1. Option to simulate a NEW vehicle check-in
    new_plate = st.sidebar.text_input("Simulate Entry Plate:", "KA-01-MJ-9999")
    if st.sidebar.button("🟢 Check-In New Vehicle"):
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute(
    ...
    if not active_vehicles.empty:
        ...
        if st.sidebar.button("🚪 Process Exit & Generate Invoice"):
...
<body>
  <div class="card">
    ...
    if os.path.exists("data/slots.json"):
        ...
        # PROMINENT INVOICE DOWNLOAD BANNER IN MAIN BODY
        ...
        while cap.isOpened():
            ...
            conn.close()
            log_table.dataframe(df, width="stretch")
